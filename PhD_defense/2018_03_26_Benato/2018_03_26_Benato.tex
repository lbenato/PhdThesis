...
\myitem $X$ resonances are narrow (1\% of $m_X$)
\end{itemize}
\end{itemize}
\end{variableblock}
\end{column}
...
\myitem curvature parameter $\tilde{k} = k / \overline{M_{Pl}}$;\\ if $\tilde{k}<1$ $\rightarrow$ $G$ narrow
%\end{itemize}
\end{itemize}
\vspace*{-0.4cm}
\begin{center}
%\includegraphics[width=0.4\textwidth]{hvt-wp-brs.pdf}
\includegraphics[width=0.42\textwidth]{../../figures/1404_0102_fig.pdf}
%\includegraphics[width=0.8\textwidth]{Brane.png}
\end{center}
\end{variableblock}
\end{column}
...
%\end{itemize}
\vspace*{0.1cm}
\item {Heavy} ($\sim$1 TeV) {\color{green!60!black}$X \rightarrow V_{\text{had}} Z_{\text{inv}}$}: Lorentz boost
...
%\end{itemize}
\myitem {\color{red!30!yellow} $Z_{\text{inv}} \rightarrow \nu \nu$}: 
%\item 
...
\myitem Trigger efficiency measured on independent data samples
\item $W \rightarrow \mu \nu$ topology and 1 large-cone jet
\item[1.] W leptonic decay $\rightarrow$ true \MET
\item[2.] $W \rightarrow \mu \nu$ $\rightarrow$ orthogonal dataset w.r.t. the signal region
\item[3.] large-cone jet requirement $\rightarrow$ kinematics similar to the signal region
\end{itemize}
\end{variableblock}
\end{column}
...
\begin{itemize}
\item $n$-subjettiness ($\tau_n$): degree to which a jet can be regarded as $n$-prongs
\item Re-clustering of jet constituents with the $k_T$ algorithm, forced to return $n$ subjets
...
\item b-tagging: MVA approach combining tracker, vertices and jet informations
%\end{itemize}
...
\begin{itemize}
\small
\item[1.] Fit the whole MC spectra of $m_j$ for all the backgrounds with physically motivated functions
\item[2.] Fix the parameters of the secondary backgrounds
\end{itemize}
%\vspace*{-0.6cm}
\small
...
\item[3.] Fit the $m_j$ spectrum in data SB to extract $V$+jets parameters (free to float)
\item[4.] Calculate the event yield in the SR
\end{itemize}
%\vspace*{-0.6cm}
\small
...
\begin{itemize}
\small
\item Split the low sideband into two sub-regions, 30-50 GeV (LSB) and 50-65 GeV (validation SR)
\item Predicted background in the validation SR is in agreement wih data
...
\item Shape $m^T_{VZ}$ of V+jets background parametrized with alternative functions (main {\tt ExpTail}; alternative {\tt ExpN}) 
\item The shape discrepancy (main/alternative function) does not impact the final result
\end{itemize}
\vspace*{-0.6cm}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height = 0.36 \textheight]{../../plotsAlpha_tesi/XVZnnlp/AlphaBiasSR.pdf}};
\node[align=center,red!50!black,font={\small\bfseries}] at (2.6,1.8) {SR predictions LP};
\end{tikzpicture}
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height = 0.36 \textheight]{../../BackgroundFunctionValidation/Exclusion_XWZInv_main_vs_alt_LPHP_test.pdf}};
\node[align=center,red!50!black,font={\small\bfseries}] at (3.0,1.8) {W' exclusion limits};
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{center}
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height = 0.36 \textheight]{../../plotsAlpha_tesi/XVZnnhp/AlphaBiasSR.pdf}};
\node[align=center,red!50!black,font={\small\bfseries}] at (2.6,1.8) {SR predictions HP};
\end{tikzpicture}
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[height = 0.36 \textheight]{../../BackgroundFunctionValidation/Exclusion_XWZInv_main_vs_alt_comb_test.pdf}};
\node[align=center,red!50!black,font={\small\bfseries}] at (3.0,1.8) {W' exclusion limits};
\end{tikzpicture}
\end{center}
\end{column}
...
\begin{variableblock}{\small Parametrization}{bg=gray!5,fg=black}{bg=gray!10,fg=red!80!black}
\begin{enumerate}
\small
\item Aim: comparing signal and background distributions in the interval 1--4 TeV
\item Few signal MC simulations available $\rightarrow$ model distributions (continuous parametrization as a function of $m_{VZ}$) 
\item $m_{VZ}^T$ signal distributions modelled as Crystal Ball functions: gaussian (2 D.O.F.) with power-law tails (2 D.O.F.) ({\color{red!70!black} shape parameters}) and yield (1 D.O.F.) ({\color{green!70!black} normalization parameter})%for Low Purity and High Purity categories
%\item {\color{green!70!black} normalization parameter} from a linear interpolation, as a function of $m_{VZ}$
%\item {\color{red!70!black} shape parameters} fitted with a 1st-3rd degree polynomial, as a function of $m_{VZ}$
\end{enumerate}
\end{variableblock}
...
\begin{variableblock}{\small Modified frequentist approach}{bg=gray!5,fg=black}{bg=gray!10,fg=red!80!black}
\begin{itemize}
\item Signal contribution in data: modified frequentist approach ($CL_s$ criterion)
\item Based on likelihood-ratio test statistics
\item 95\% C.L. upper limit on signal strength
\item Local p-value scans (incompatibility with background-only hypothesis)
\item Systematic uncertainties as nuisance parameters: log-normal priors (positively defined nuisances), profiled during the minimization
\item Likelihood fits performed per purity category and combining the categories
\vspace*{0.5cm}
\item Fit diagnostic: pre/post-fit nuisance pulls $(\hat{\theta} - \theta_0) / \Delta \theta$ compared
\end{itemize}
\end{variableblock}
...
\begin{frame}
\frametitle{Results}
\framesubtitle{Local p-value scans}
\small
\vspace*{-0.3cm}
\begin{itemize}
...
%\begin{tikzpicture}
%\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=.45\textwidth]{../../plotsAlpha_tesi/Limits/Significance_XWZInv_XVZnn.pdf}%
%     \includegraphics[width=.45\textwidth]{../../plotsAlpha_tesi/Limits/pValue_XWZInv_XVZnn.pdf}};
%\node[align=center,blue,font={\footnotesize}] at (3.5,2.) {$W'$};
%\end{tikzpicture}
...
\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=.45\textwidth]{../../plotsAlpha_tesi/Limits/Significance_XWZInv_XVZnn.pdf}%
     \includegraphics[width=.45\textwidth]{../../plotsAlpha_tesi/Limits/pValue_XWZInv_XVZnn.pdf}};
\node[align=center,blue,font={\small}] at (3.5,1.8) {HVT $W'$};
\node[align=center,blue,font={\small}] at (8.5,1.8) {HVT $W'$};
\end{tikzpicture}

\begin{tikzpicture}
\node[anchor=south west,inner sep=0] (image) at (0,0) {     \includegraphics[width=.45\textwidth]{../../plotsAlpha_tesi/Limits/Significance_XZZInv_XVZnn.pdf}%
     \includegraphics[width=.45\textwidth]{../../plotsAlpha_tesi/Limits/pValue_XZZInv_XVZnn.pdf}};
\node[align=center,blue,font={\small}] at (3.5,1.8) {Bulk Graviton};
\node[align=center,blue,font={\small}] at (8.5,1.8) {Bulk Graviton};
\end{tikzpicture}
...
\begin{frame}
\frametitle{Interpretation of the results: HVT model}
\small
\vspace*{-0.3cm}
 \begin{center}
...
\begin{variableblock}{\small HVT parameter space exclusion}{bg=gray!5,fg=black}{bg=gray!10,fg=red!80!black}
\begin{itemize}
